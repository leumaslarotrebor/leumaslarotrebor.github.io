    ...
    >
      ...
      >
        ...
        {project.image ? (
          ...
          <div
            style={{ background: project.gradient || "linear-gradient(145deg,#333,#000)" }}
            className="w-full h-40 flex flex-wrap content-center justify-center gap-2 p-6 rounded-t-2xl"
          >
            {(project.tags || []).map((t) => (
              <span
                key={t}
                className="text-xs font-medium px-3 py-1.5 rounded-full bg-black/30 text-white/90 border border-white/10"
              >
                {t}
              </span>
            ))}
